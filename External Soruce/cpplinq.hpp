...
namespace cpplinq
{
    ...
    namespace detail
    {
        ...
        struct min_selector_builder : base_builder
        {
            ...
            CPPLINQ_INLINEMETHOD typename TRange::value_type build (TRange range) const
            {
                auto current = std::numeric_limits<typename TRange::value_type>::maxL ();
                while (range.next ())
                {
                    auto v = selector (range.front ());
                    if (v < current)
    ...
    template<typename TSelector>
    CPPLINQ_INLINEMETHOD detail::max_selector_builder<TSelector> maxL (
            TSelector selector
        ) CPPLINQ_NOEXCEPT
    {
    ...
    CPPLINQ_INLINEMETHOD detail::max_builder maxL () CPPLINQ_NOEXCEPT
    {
        return detail::max_builder ();
    }

    template<typename TSelector>
    CPPLINQ_INLINEMETHOD detail::min_selector_builder<TSelector> minL (
            TSelector selector
        )
    {
    ...
    CPPLINQ_INLINEMETHOD detail::min_builder minL () CPPLINQ_NOEXCEPT
    {
